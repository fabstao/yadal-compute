...
virt-install --connect qemu:///system --name $1 --memory $3 --vcpus $4 --network network=$2,model=virtio --cloud-init user-data=${BASEDIR}/user-data,meta-data=${BASEDIR}/meta-data,root-ssh-key=${BASEDIR}/$6 --import --disk ${POOL}/${VMNAME}.qcow2 --graphics vnc --os-variant centos-stream9 --noautoconsole
